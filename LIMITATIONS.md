...
- **ViewGraph is browser-only** — built-in loaders use `window`, `document`, `fetch`, and `customElements`. No Node SSR view pipeline (`isSSR` is reserved / always false in the browser env). Client adopt of server HTML is shipped (`extract` / `aura-router-ssr`); there is still no server-side Aura render. First-paint contract (adopt skips lifecycle; nested tree rules): [guide First paint](./docs/guide.md#first-paint-mpa--spa).
