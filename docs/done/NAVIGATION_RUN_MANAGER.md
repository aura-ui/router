...
**Шаг 2 —** `applyNavigationOutcome` (+ `NavigationIdentity`)` · host `applyTerminalOutcome` · coordinator `processResult`.  
**Шаг 3 —** `failure/` только модель; side effects в `navigation-outcome`.  
...
| 7 | **Terminal finalize (error/cancel/redirect/404)** | <span style="background:#16a34a;color:#fff;padding:2px 6px;border-radius:4px;font-weight:700">✓</span> | `settle` → `applyNavigationOutcome` (+ `NavigationIdentity`)` |
...
| **Terminal apply** | `applyNavigationOutcome` (+ `NavigationIdentity`)` · host `applyTerminalOutcome` | <span style="background:#16a34a;color:#fff;padding:2px 6px;border-radius:4px;font-weight:700">✓</span> |
...
| **1b — terminal apply** | <span style="background:#16a34a;color:#fff;padding:2px 6px;border-radius:4px;font-weight:700">✓</span> `applyNavigationOutcome` (+ `NavigationIdentity`)` |
...
Единая точка **apply** (side effects) — функции в `navigation-outcome.ts`, **не** class/deps-порт и **не** Pulse.  
...
Pre-match
  → applyNavigationOutcome(failure.toResult(), nav, …)  ✓
...
AuraRouter
  └─ AuraRoutingEngine                                    ✓
       ...
       applyNavigationOutcome (+ NavigationIdentity)              ✓
       NavigationProvider                                 ✓
...
NavigationTransaction  intent lifecycle: full + rollback; `tx.engine` by design
                      ✓
Terminal apply        applyNavigationOutcome (+ NavigationIdentity)
                      ✓ (bus в Pulse.settle)
...
1. <span style="background:#16a34a;color:#fff;padding:2px 6px;border-radius:4px;font-weight:700">✓</span> `applyNavigationOutcome` (+ `NavigationIdentity`)` (функции).
...
|------|-----------|--------|
| `core/navigation/navigation-outcome.ts` | функции apply | <span style="background:#16a34a;color:#fff;padding:2px 6px;border-radius:4px;font-weight:700">✓</span> |
