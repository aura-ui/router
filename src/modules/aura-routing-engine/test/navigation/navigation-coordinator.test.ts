...
describe('NavigationCoordinator', () => {
  ...
  describe('planning вЂ” noop', () => {
    ...
    it('ignores duplicate href while in flight', async () => {
      ...
      const first = engineNavigate(coordinator, navOptions({ from: home, to: about, href: '/about' }));

      const second = engineNavigate(coordinator, navOptions({ from: home, to: about, href: '/about' }));
  ...
  describe('planning вЂ” run', () => {
    ...
    it('does not clear a newer activeTransaction when a superseded run settles', async () => {
      ...
      const coordinator = new NavigationCoordinator(createCoordinatorMockHost());
