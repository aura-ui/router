...
| `match/` | URL matching (`url-matcher.ts`), `resolveNavigationTarget`, `MatchedRouteInfo`. |
...
sequenceDiagram
  ...
  participant Match as resolveNavigationTarget
  ...
  alt no match
    Engine->>Engine: runNotFoundExitCleanup + finalizeNotFoundNavigation
