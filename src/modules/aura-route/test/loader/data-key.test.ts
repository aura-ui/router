import { dataCacheKey } from '../../../aura-routing-engine/core';
...
describe('dataCacheKey', () => {
  ...
  it('separates cache slots for same ref on different pathnames', () => {
    ...
    expect(
      dataCacheKey(desc, { pathname: '/user/1', pattern: '/user/:id' } as any),
    ).not.toBe(
      dataCacheKey(desc, { pathname: '/user/2', pattern: '/user/:id' } as any),
    );
  });
});
