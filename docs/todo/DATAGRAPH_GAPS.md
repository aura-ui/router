...
**Не путать с кэшем `aura-cache-store`:** механизм «отдай старое → пометь протухшим → догрузи в фоне» (SWR) **уже есть** и DataGraph им пользуется — через `AuraResolvableCache` + `staleTime` (по умолчанию 30 с), но **только** если на маршруте `preserve="data"` (см. п. 4). Это инфраструктура кэша, а не выбор режима per route.

| Что | Есть в коде? |
|-----|--------------|
| SWR: stale → сразу старое + фоновый fetch | ✓ `aura-cache-store`, в DataGraph при `preserve="data"` |
...
| | Вопрос |
|--|--------|
| `preserve="data"` | запоминать ли результат `load` в кэш вообще |
...
Без `preserve="data"` кэша нет — hook и так дергается каждый раз; это не «shouldRevalidate всегда true», а **кэширование выключено**.
...
`shouldRevalidate` — **в первую очередь программный API**, не HTML-attr как `scroll` или `preserve`. В разметке не выразить логику «после POST — да, при Back — нет».
...
preserve="data"          — кэшировать ли вообще (п. 4)
```
...
## 4. Кэш данных только если явно включить `preserve="data"`

### Сейчас в коде

Данные из `load` **запоминаются** только у маршрутов с `preserve="data"` или `preserve="all"`.  
...
С другой — для вложенных layout приходится **вручную** вешать `preserve="data"` на каждый родительский маршрут, иначе не сработает сценарий «родитель живёт — его данные не перезапрашиваем».

### Как должно быть (минимум)

- Чётко описать в доке: без `preserve="data"` кэша нет.
- Показать пример nested layout с этим атрибутом.
- Опционально: наследуемый default на `<aura-router preserve="data">`, чтобы не дублировать на каждом layout.
...
<aura-router stale-time="30000" gc-time="300000">
  <aura-route path="/notifications" stale-time="10000" load="fetch-notifs" preserve="data" />
  <aura-route path="/profile" stale-time="300000" load="fetch-profile" preserve="data" />
</aura-router>
```
...
**Условие:** имеет смысл только вместе с **`preserve="data"`** (п. 4) — без кэша load-данных TTL не к чему применять.
...
`AuraRouter.configure({ dataCache: ... })` настраивает **второй** кэш (разметку), не DataGraph.

### Зачем это нужно

Страница с `preserve="view"` и `html-src` после первого визита всегда показывает **тот же HTML**, даже если на сервере partial уже другой. Для данных это уже почти решено; для разметки — нет.
...
| 4 | `preserve="data"` по умолчанию выключен | высокий (удобство) | да |
