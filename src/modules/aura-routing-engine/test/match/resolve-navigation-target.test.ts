...
import { MAX_REDIRECT_HOPS, resolveNavigationTarget } from '../../core/match/resolve-navigation-target';
import { resolveRedirectHref } from '../../core/match/resolve-redirect-target';
import {
  buildTreeFromDom,
...
describe('resolveNavigationTarget', () => {
  ...
  it('returns unmatched when no route matches', () => {
    ...
    expect(resolveNavigationTarget(matcher, '/missing', matchableNodes)).toEqual({ kind: 'unmatched' });
  });
  ...
  it('follows top-level absolute redirect to final leaf', () => {
    ...
    const target = resolveNavigationTarget(matcher, '/settings', matchableNodes);
  ...
  it('follows nested relative redirect from index child', () => {
    ...
    const target = resolveNavigationTarget(matcher, '/app', matchableNodes);
  ...
  it('follows multi-hop redirect chains', () => {
    ...
    const target = resolveNavigationTarget(matcher, '/a', matchableNodes);
  ...
  it('detects redirect cycles', () => {
    ...
    expect(resolveNavigationTarget(matcher, '/a', matchableNodes)).toEqual({
      kind: 'redirect-error',
      code: 'redirect-cycle',
      href: '/a',
    });
  ...
  it('detects redirect depth overflow', () => {
    ...
    expect(resolveNavigationTarget(matcher, '/hop-0', matchableNodes)).toEqual({
      kind: 'redirect-error',
  ...
  it('returns unmatched when redirect target has no route', () => {
    ...
    expect(resolveNavigationTarget(matcher, '/entry', matchableNodes)).toEqual({ kind: 'unmatched' });
  });
  ...
  it('preserves search and hash from the original href on the final leaf', () => {
    ...
    const target = resolveNavigationTarget(matcher, '/settings?tab=1#panel', matchableNodes);
