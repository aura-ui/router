...
> **Статус:** черновик / решение по именам · **в коде as-is: `update`** · `guard` / `ready` / `unmount` — в коде ✓  
...
| Уровень | Attrs | Доля маршрутов | Где в docs |
|---------|-------|----------------|------------|
...
| **Presentation** | `transition-in`, `transition-out`, `transition-order` | опционально | отдельная ось, не lifecycle |
| **Super advanced** | `detach`, `destroy`, `restore` | только `preserve` / keep-alive | отдельная секция, не в quick start |
...
| Attr | Роль | Blocking? | Смысл одной фразой |
|------|------|-----------|-------------------|
...
| Attr | Роль | Почему не в lifecycle |
|------|------|------------------------|
...
| `preserve.view` | Поведение view | Будущий attr |
|-----------------|----------------|--------------|
| `true` | `handle.detach()` — DOM снят с outlet, subtree в ViewCache | **`detach`** |
| `false` | `handle.destroy()` — полная очистка | **`destroy`** |

#### Enter (reattach)

| Условие | Поведение view | Будущий attr |
|---------|----------------|--------------|
| ViewCache hit, `reattachContent` | DOM возвращён в outlet без полного render | **`restore`** |
...
ctx.preserved: { view: boolean; data: boolean }
...
> **Порядок as-is:** на фазе `left`/`unmount` сначала `route.onLeft()` → detach/destroy, затем user hooks. Логика **до** снятия DOM — в **`leave`** (blocking). Открытый вопрос: нужен ли pre-teardown hook — см. [OUTLET_AND_RENDER.md](./OUTLET_AND_RENDER.md).
...
| Концепция | Vue Router | TanStack / RR7 | Angular | Aura (план) |
|-----------|------------|----------------|---------|-------------|
...
| Keep-alive off/on | `onDeactivated` / `onActivated` | — | — | **`detach` / `restore`** (planned) |
...
| # | Концепция | У кого | У Aura | Как думать в будущем |
|---|-----------|--------|--------|----------------------|
| 1 | **Глобальные guards** `beforeEach` / `afterEach` | Vue | ✗ TS API; ~ inherit `guard`/`ready` на router | Router-level API или convention inherit; **не новая фаза** |
...
| 4 | **`action` (мутации POST/form)** | Remix / RR7 | ✗ | Осознанный gap для WC/HTML-first; hooks вне router или отдельный слой, не lifecycle attr |
...
| 8 | **Pending / defer UI** (`pendingMs`, `defer`) | TanStack, RR | `loading-template` ✓; без defer policy | UX на **`load`**, не новая фаза |
| 9 | **Полный param-update** | Vue `beforeRouteUpdate` | `update` — лёгкий shortcut | Опция «full cycle на update» через policy/attr-modifier, не новое имя фазы |
...
| Слот | Вариант A | Вариант B | Комментарий |
|------|-----------|-----------|-------------|
| post-commit (core) | **`ready`** | **`entered`** | `ready` — шире («route готов к работе»), ближе к `document.ready`; `entered` — route-native, пара к «входу» |
| cleanup (advanced) | **`unmount`** | **`left`** | `unmount` — явнее для dev'ов (React/Svelte); `left` — короче, наследие Navigo |
| same-route (advanced) | **`update`** | **`reenter`** | `update` — как Vue `beforeRouteUpdate`; `reenter` — уже в коде и docs |
...
| As-is (код / docs) | → Рекомендуемое | Примечание |
|--------------------|-----------------|------------|
| `enter` | **`guard`** | `enter` не кодирует blocking |
| `after` / `afterHook` / `onAfter` | **`ready`** или **`entered`** | `after` слишком абстрактно |
| `left` / `onLeft` | **`unmount`** или **`left`** | |
| `reenter` / `onReenter` | **`update`** или **`reenter`** | |
...
`ctx.phase` (TS) — camelCase: `guard`, `ready`, `unmount`, `update` (или `entered`, `left`, `reenter` при выборе альтернативы).
...
- [ ] Финальный выбор: `ready` vs `entered` для core post-commit
- [ ] Финальный выбор: `unmount`/`update` vs `left`/`reenter` для advanced
...
- [ ] Порядок: user hooks vs `onLeft()` (pre/post teardown)
- [ ] Deprecated aliases на переходный период (`enter` → `guard`, `after` → `ready`, …)
- [ ] Обновить [ROUTE_API_V3.md](./ROUTE_API_V3.md) после фиксации имён
- [ ] Рефакторинг: `phase-registry.ts`, `aura-route.ts` attrs, тесты, `PHASE_NAMING.md`
